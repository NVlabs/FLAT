    ...
    for key in keys:
        ...
        if not os.path.isfile(key):
            gdd.download_file_from_google_drive(
                file_id=file_dir[key]['file_id'], dest_path=key, unzip=True)
...
def main():
    ...
    if not os.path.exists(folder):
        os.mkdir(folder)
    if not os.path.isfile(folder + 'params.zip'):
        gdd.download_file_from_google_drive(file_id=param_id,
    ...
    lists = [
        key for key in file_dir.keys() if (flat_flg in key) * ('.txt' in key)
    ]
    if 'all' in args.name:
        list_flgs = lists
    else:
    ...
    if flat_flg == 'trans_render':
        ...
        keys = [
            key for key in file_dir.keys() if (key.split('/')[1] == flat_flg)
        ]
        ...
        for i in range(len(list_flgs)):
            ...
            if filename in file_dir.keys():
                ...
                keys = [
                    key for key in file_dir.keys()
                    if (key.split('/')[-1] in data_list) and (
                        key.split('/')[1] == flat_flg)
                ]
                batch_download(keys, file_dir)
