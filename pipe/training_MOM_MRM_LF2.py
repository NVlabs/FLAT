...
def select_objects():
    ...
    f = open('../FLAT/kinect/list/motion-foreground.txt','r')
    message = f.read()
    files = message.split('\n')
    trains = files[0:-1]
...
def data_augment_axial(name, test_dir, tof_cam):
    f = open('../FLAT/kinect/list/'+name+'.txt','r')
    message = f.read()
    files = message.split('\n')
    scenes = files[0:-1]
    scenes = [test_dir[0:-1]+'-full/'+scene[-23:-7] for scene in scenes]
...
if __name__ == '__main__':
    ...
    f = open('../FLAT/kinect/list/multi-reflect.txt','r')
    message = f.read()
    files = message.split('\n')
    trains = files[0:-1]
